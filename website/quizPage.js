...
/*function prev() {
    // if it's not the first or second question, index-1 to  go to the previous question
    //also enables the 'next' button in case we were on the last question and next was disabled
    checkAnswer();
    console.log("prev: " + Q);
...
//if it is the second question, go to the first question (index-1) and disable the 'prev' button
    else if (Q==0) {
        document.getElementById("prev").disabled = true;
    }

    //current issue: HOW TO KEEP TRACK OF SCORES while allowing for users to go back and forth b/w questions ...
    questionText.innerHTML = (Q+1) + ". " + "<strong>" + quizData[Q].question + "</strong>";
    var answers = [];
    var output = [];
    for (letter in quizData[Q].answers) {
